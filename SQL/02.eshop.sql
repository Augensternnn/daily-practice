...
-- SPU：产品的某个型号，不涉及具体规格。例：iPhone11，包含一些共有属性，如：CPU、屏幕材质等
-- SKU：具体到规格的具体某个商品，如：128G、黑色、无需合约版的iPhone11
...
from
	sku_attr_value sav
  join sku on sku.id = sav.sku_id
	join spu on sku.spu_id = spu.id
	join attr_value av on sav.attr_value_id = av.id
	join attr_key ak on av.attr_key_id = ak.id
where 
	spu.name like 'iphone%' order by spu_id,sku;
	and av.name = '黑色';